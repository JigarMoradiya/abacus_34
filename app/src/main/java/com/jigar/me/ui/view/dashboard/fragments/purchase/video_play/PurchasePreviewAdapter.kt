...
class PurchasePreviewAdapter(
    ...
    RecyclerView.Adapter<PurchasePreviewAdapter.FormViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): FormViewHolder {
        val binding =
            RawPurchasePreviewBinding.inflate(parent.context.layoutInflater, parent, false)
    ...
    override fun onBindViewHolder(holder: FormViewHolder, position: Int) {
        ...
        with(holder.binding) {
            ...
            if(data.sku == PRODUCT_ID_Subscription_Year1 || data.sku == PRODUCT_ID_Subscription_Year1_Offer ){
                cardMain.setCardBackgroundColor(ColorStateList.valueOf(ContextCompat.getColor(context,R.color.purple_50)))
                cardMain.setStrokeColor(ColorStateList.valueOf(ContextCompat.getColor(context,R.color.purple_700)))
            }else{
                cardMain.setCardBackgroundColor(ColorStateList.valueOf(ContextCompat.getColor(context,R.color.grey_100)))
                cardMain.setStrokeColor(ColorStateList.valueOf(ContextCompat.getColor(context,R.color.grey_700)))
            }
            sku = data
            cardMain.onClick {
            ...
            if (discountPer > 0){
                if(data.sku == PRODUCT_ID_Subscription_Year1_Offer){
            ...
            if(data.sku == PRODUCT_ID_Subscription_Year1 || data.sku == PRODUCT_ID_Subscription_Year1_Offer){
                listData.find { it.sku == PRODUCT_ID_Subscription_Month1 }.also {
                    if (it != null){
    ...
    class FormViewHolder(
        itemBinding: RawPurchasePreviewBinding
    ) :
        RecyclerView.ViewHolder(itemBinding.root) {
        var binding: RawPurchasePreviewBinding = itemBinding
    }
