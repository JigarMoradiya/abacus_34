...
@HiltViewModel
class KakuroHomeViewModel @Inject constructor(
    private val prefManager: AppPreferencesHelper
) : ViewModel() {

    val progress = KakuroProgress(prefManager)

    private val _selectedDifficulty = MutableStateFlow(
        CommonDifficulty4.fromName(prefManager.getCustomParam("kakuroDiff", CommonDifficulty4.easy.name))
    )
    ...
    fun selectDifficulty(difficulty: CommonDifficulty4) {
        ...
        prefManager.setCustomParam("kakuroDiff", difficulty.name)
    }
