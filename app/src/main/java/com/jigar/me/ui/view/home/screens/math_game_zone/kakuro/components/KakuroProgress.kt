...
class KakuroProgress(private val prefManager: AppPreferencesHelper) {

    private fun key(difficulty: CommonDifficulty4) = "kakuroStars_${difficulty.name}"

    fun stars(difficulty: CommonDifficulty4): IntArray {
        val raw = prefManager.getCustomParam(key(difficulty), "")
        val out = IntArray(KakuroConfig.LEVEL_COUNT)
        if (raw.isNotEmpty()) {
    ...
    fun nextLevel(difficulty: CommonDifficulty4): Int {
        ...
        return if (i == -1) KakuroConfig.LEVEL_COUNT else i + 1
    }
}
