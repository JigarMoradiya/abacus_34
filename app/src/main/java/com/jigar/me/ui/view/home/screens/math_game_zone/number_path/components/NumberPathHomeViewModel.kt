...
@HiltViewModel
class NumberPathHomeViewModel @Inject constructor(
    private val prefManager: AppPreferencesHelper
) : ViewModel() {

    val progress = NumberPathProgress(prefManager)

    private val _selectedDifficulty = MutableStateFlow(
        CommonDifficulty4.fromName(prefManager.getCustomParam("numberPathDiff", CommonDifficulty4.easy.name))
    )
    ...
    fun selectDifficulty(difficulty: CommonDifficulty4) {
        ...
        prefManager.setCustomParam("numberPathDiff", difficulty.name)
    }
